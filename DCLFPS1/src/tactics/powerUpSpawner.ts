...
export class PowerUpSpawner extends Entity
{
    ...
    constructor(id:string,spawnPosition: Vector3)
    {
        ...
        this.addComponent(new GLTFShape("models/healthpickup.glb")).withCollisions = false
    ...
    pickup(){
        ...
        //adjust
        REGISTRY.player.setEnablePowerUp('normal',true,REGISTRY.player.maxAmmo,CONFIG.bullet_POWERUP_DURATION_SECONDS)
    ...
    checkState()
    {
        if(this.activateTime <= REGISTRY.getGameTime())
        {
            ...
            log("The fire is no longer affected by bullets")
            this.hide()
        }
    }
