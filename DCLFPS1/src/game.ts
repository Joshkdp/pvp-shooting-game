...
import * as battleConn from 'src/bullet-fight/connection/onConnect';
import { BattleDataOptions } from './bullet-fight/connection/state/server-state-spec';
...
import { initSceneMgr } from './bullet-fight/sceneManager';
...
import { cacheUserData } from './bullet-fight/init';
...
function AddDelay(){
  ...
  startRefreshTimer(2000, (val)=> {
    ...
    if(GAME_STATE.gameConnected ==='connected'){
      ...
      REGISTRY.SCENE_MGR.bulletArena.exitBattle();
...
async function init(){
  ...
  REGISTRY.onConnectActions=(room:Room<any>,eventName:string)=>{
      battleConn.onbulletFightConnect(room)
  }
