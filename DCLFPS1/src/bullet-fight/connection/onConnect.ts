...
//import { fpsTrenchController } from "src/tactics/fpsTrench";
...
function updateBattleData(battleData: clientState.BattleState) {
  ...
  switch (battleData.status) {
    ...
    case "starting":
      ...
      //Constants.SCENE_MGR.changeTobulletArena();
      ...
      REGISTRY.SCENE_MGR.bulletArena.battleAboutToStart()
      ...
      //Constants.SCENE_MGR.bulletArenaScene.startingBattle();
    ...
    case "started":
      ...
      REGISTRY.SCENE_MGR.bulletArena.startBattle()
    ...
    case "ended":
      ...
      REGISTRY.SCENE_MGR.bulletArena.endBattle()
...
function updateTrackFeature(trackFeat: clientState.ITrackFeatureState) {
  ...
  switch(trackFeat.type){
    ...
    case 'fireplace':
      ...
      break;
    // case 'trench':
    //   const trench = fpsTrenchController.getTrenchById(trackFeat.name)
    //   if(trench !== undefined){
...
export async function onbulletFightConnect(room: Room) {
...
function onLevelConnect(room: Room<clientState.BattleRoomState>) {
  ...
  REGISTRY.SCENE_MGR.bulletArena.onConnect( room )
  ...
  room.onMessage("player.death", (data:serverStateSpec.PlayerGiveDamageDataState) => {
    ...
    if (data.playerIdTo == room.sessionId) {
      ...
      REGISTRY.SCENE_MGR.bulletArena.thisPlayerDeath(data)
    }else{
  ...
  room.state.players.onAdd = (player: clientState.PlayerState, sessionId: string) => {
    ...
    player.battleData.listen("teamId", (carModelId: string) => {
      ...
      if (player.sessionId != REGISTRY.player.sessionId) {
        /*const enemy = ENEMY_MGR.getPlayerByID(player.sessionId);
        if (enemy) {
          enemy.updateCarModelById(carModelId, "bullet");
        }*/
      }
      //refreshLeaderboard();
    });
