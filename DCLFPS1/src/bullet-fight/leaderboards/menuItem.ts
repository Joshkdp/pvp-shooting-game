...
import { AnimatedItem } from "src/utils/simpleAnimator";
import * as resource from "src/resources/resources";
...
export class LevelMenuItem extends MenuItem {
  ...
  constructor(
    ...
    //   })
    // );
    // //this.itemRoot.addComponent(resource.smallCardShape);//backdrop card
    ...
    //highlight on click
    this.highlightFrame = new Entity(this.uuid+".highlighter");
    ...
    //there is some bug where it wont render it more than once?!?!?
    this.highlightFrame.addComponent(resource.smallCardHighlightShape);//higlight card
    this.highlightFrame.addComponent( 
      new AnimatedItem(
        {
          position: new Vector3(0, 0, 0),
          scale: new Vector3(0, 0, 0),//scale: new Vector3(1, 1, 1),/////
          rotation: Quaternion.Euler(0, 0, 0),
        },
        {
          position: new Vector3(0, 0, 0),
  ...
  select() {
    ...
    if (!this.selected || this.reSelectCooldownPassed()) {
      ...
      const anim = this.highlightFrame.getComponent(AnimatedItem)
      anim.isHighlighted = true;
      this.titleText.color = Color3.White();
      this.titleText.font = new Font(Fonts.SansSerif_Heavy);

      log("select.font heavy",this.selected)
    }
  }
  deselect(_silent?: boolean) {
    if (this.selected) {
