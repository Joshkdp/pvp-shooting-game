...
//import { fpsTrenchController, fpsTrenchInfo } from 'src/tactics/fpsTrench'
...
//preload dummy
const ballShape = new GLTFShape('models/cyanball.glb')
    ...
    for (let ball of this.ballManager.balls) {
      if (ball.active) {
       ...
       // ball.moveVector.y -= ball.gravity * dt * 0.1
        ...
        let rayCastId = CONFIG.bullet_USE_RAYCAST_ID_PER_BALL ? ball.rayCastId : undefined //ball.myBall ? 1 : 2 
        ...
        this.physicsCast.hitFirst(rayBallCollide, (e) => {
          if (e.didHit ) {
            ...
            switch (e.entity.meshName){
              ...
              // {
              //   log ('BallManager',METHOD_NAME,ball.id,"fpsTrench was HIT")

              //   if(engine.entities[e.entity.entityId].hasComponent(fpsTrenchInfo)){
              //     let id = engine.entities[e.entity.entityId].getComponent(fpsTrenchInfo).id

              //     fpsTrenchController.UpdateHealth(id, ball.createDamagePayload())
