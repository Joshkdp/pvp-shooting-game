...
export class Player extends BasePlayer{
  ...
  maxAmmo: number = CONFIG.bullet_MAX_AMOUNT
  ...
  playerHitFeedback(updateType:UpdateType){
    ...
    const battleActive = REGISTRY.SCENE_MGR.bulletArena.isArenaActive
    if(!battleActive){
  ...
  setInfpsPickupZone(val: boolean) {
    this.isInfpsPickupZone = val
    ...
    ui.displayMakebulletInstructions(val)
  }
  ...
  collectAmmo() {
    ...
    if (this.isInfpsPickupZone) {
      

      this.ammoSys.isActive = true       
      if(!CONFIG.bullet_AUTO_COLLECT_ENABLED) triggerEmote({ predefined: 'crafting' as any })
    }
    else{
      DisplayCursorMessage("CRAFTING NOT ALLOWED HERE", "USE fpsY AREAS", 2)
    }
  ...
  update(dt: number) {
    if (this.isActive) {
      if (this.elapsed < this.cooldown) {
        ...
        this.elapsed = 0
        if(CONFIG.bullet_TRIGGER_EMOTES_ENABLED) triggerEmote({ predefined: 'crafting' as any })
      }
    }
  }
}
...
export class bulletCooldownSystem {
  elapsed: number = 0
  cooldown: number = CONFIG.bullet_COOLDOWN_SECONDS
...
//engine.addSystem(new bulletCooldownSystem(player))
