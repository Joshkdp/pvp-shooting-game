...
import * as battleClientSpec from 'src/bullet-fight/connection/state/client-state-spec'
import { BattleData } from './modules/battleData'
...
export class GameState{
  ...
  //raceData:RaceData
  battleData:BattleData//TODO MAKE NEW
  //trackData:TrackData
  ...
  getBattleRoom():Room<battleClientSpec.BattleRoomState>{
    if(this.gameRoom && this.gameRoom.name == CONFIG.GAME_bullet_ROOM_NAME){
      return this.gameRoom as Room<battleClientSpec.BattleRoomState>
    }else if(this.prevGameRoom && this.prevGameRoom.name == CONFIG.GAME_bullet_ROOM_NAME){
      return this.prevGameRoom as Room<battleClientSpec.BattleRoomState>
    }
    return undefined
  }
  getGameRoom<T>(){
