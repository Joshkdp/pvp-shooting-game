...
import PlayFab from "src/playfab_sdk/PlayFabClientApi";
import * as PlayFabSDKWrapper from  'src/playfab_sdk/index'
import { EntityTokenResponse, GetLeaderboardResult, GetPlayerCombinedInfoResult, GetPlayerCombinedInfoResultPayload, LoginResult, TreatmentAssignment, UserSettings } from 'src/playfab_sdk/playfab.types'; 
import * as ui from '@dcl/ui-scene-utils'

//import * as PlayFabSDK from 'playfab-sdk'

import { GAME_STATE } from 'src/state';
import { getAndSetUserData, getAndSetUserDataIfNullNoWait, getRealmDataFromLocal, getUserDataFromLocal, setRealm } from 'src/utils/userData';
import { isNull, notNull, preventConcurrentExecution } from 'src/utils/utilities';
...
import { LeaderBoardManager } from 'src/bullet-fight/leaderboards/menu';
...
function handleCustomIdError(json:any|Error){
    ...
    if(json instanceof Error){
        ...
        REGISTRY.Game_2DUI.showLoginErrorPrompt( undefined,"There was a an error signing in\n" + json.message  )
    }else{
        GAME_STATE.playerState.loginFlowState='customid-error'
...
async function  updateLoginInfoFromResult(result:LoginResult) {
    log("updateLoginInfoFromResult",result)
    ...
    if(notNull(result) && isNull(result.error)){
        ...
        let dclUserData = getUserDataFromLocal()
        if (dclUserData === undefined) {
            dclUserData = await getAndSetUserData()
        }
        if (!getRealmDataFromLocal()) {
            await setRealm()
        }
        
        if(CONFIG.PLAYFAB_ENABLED && dclUserData !== null ){

            PlayFabSDKWrapper.UpdateUserTitleDisplayName( {
                DisplayName: createUserTitleDispalyName(dclUserData)
                }
            ).then(()=>{
                ...
                userData["ethPublicKey"] = dclUserData.publicKey
                userData["userId"] = dclUserData.userId
        ...
        //debugger 
        REGISTRY.rewardPrompt.updateRewards(result.InfoResultPayload)
...
export function refreshUserData(delay?:number){
    ...
    utils.setTimeout(delay!== undefined ? delay : 1000,()=>{  
        fetchRefreshPlayerCombinedInfo().then((val:GetPlayerCombinedInfoResult)=>{
            ...
            REGISTRY.rewardPrompt.updateRewards(val.InfoResultPayload)
        })
    })
}
                ...
                InfoRequestParameters: 
                    {
                        ...
                        UserReadOnlyDataKeys: [XMAS_DAYS_PLAYED]
                    },
