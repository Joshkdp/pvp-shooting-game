...
import { PlayerHealthDataState, PlayerState, PlayerStatsKillDataState as PlayerStatsDamageDataState, TeamMemberState, TeamState } from "../bulletState/server-state";
import * as serverStateSpec from "../bulletState/server-state-spec";
...
export class BattleRoom extends BaseBattleRoom {
    ...
    onCreate(options:OnConnectOptions) {
        ...
        this.onMessage('throwBall', (client, throwBallData:serverStateSpec.BallThrowData) => {
            ...
            //console.log(player.name, ' threw a bullet ', message.pos, message.dir, message.force)
            log(CLASSNAME,this.roomId,METHOD_NAME, player.sessionId, player.userData.name, ' threw a bullet ', throwBallData.pos, throwBallData.dir, throwBallData.force,"",player.battleData.teamId,player.battleData._teamRef);
        })
